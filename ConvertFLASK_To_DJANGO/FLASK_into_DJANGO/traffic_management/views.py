...
from Data_Recieving.implimentingsockets import *    
...
@csrf_exempt
def latest_hour_TJM_data(request):
  print("Hi----------------jii")
  if request.method == "GET":
    tjm_data_list = list(TRAFFICJAM_MANAGEMENT_DATA.aggregate([
    {
        "$match": {
        "date": Gettodaydate #,
...
def test_todays_data(request):
    result = {"message":"Something ent wrong with 'test_todays_data'.", "success":False}
    if request.method == "GET":
        ...
        if test_data:
            result= {"message": "Test query successful", "data": parse_json(test_data), "success": True}
        else:
            result= {"message": "No data found for today's date", "success": False}
...
@csrf_exempt
def overall_to222days_TJM_data(request):
    result = {'success': False, 'message': 'Something went wrong with overall_to222days_TJM_data.'}
    if request.method == "GET":
        tjm_data = list(TRAFFICJAM_MANAGEMENT_DATA.aggregate([
        {
        "$match": {
            "date": Gettodaydate
        }
...
@csrf_exempt
def live_traffic_jam_data(request):
...
def live_trafficjam_data(request):
...
def get_rois_list(request):
    ...
    if request.method == "GET":
        ...
        TJM_rois = TRAFFICJAM_MANAGEMENT_DATA.distinct("roi_name", {"date": Gettodaydate})
        if TJM_rois:
            if len(TJM_rois) != 0:
...
@csrf_exempt
def datewise_rois_list(request):
    ret = {'success': False, 'message': 'Something went wrong.'}
    if request.method == "POST":
        jsonobject = json.loads(request.body)
        if jsonobject == None:
        ...
        if not missing_key:
            ...
            if output:
                ...
                TJM_roi_names_list = list(TRAFFICJAM_MANAGEMENT_DATA.distinct("roi_name", {'timestamp':{'$gte':from_date, '$lte': to_date}}))
...
def get_cameralist(request):
    ...
    if request.method == "GET":
        # TJM_cameras_Data = list(mongo.db.TRAFFICJAM_MANAGEMENT_DATA.find({"date":Gettodaydate},{'_id':0, 'dashboard_log':0}).sort('timestamp', -1))
        TJM_cameras = list(TRAFFICJAM_MANAGEMENT_DATA.distinct("camera_name", {"date": Gettodaydate}))
    ...
    if request.method == "POST":
        ...
        if not missing_key:
            ...
            if output:
                ...
                from_date = jsonobject['from_date']
                to_date = jsonobject['to_date']
                TJM_cameras_list = list(TRAFFICJAM_MANAGEMENT_DATA.distinct("camera_name", {'timestamp':{'$gte':from_date, '$lte': to_date}}))
...
@csrf_exempt
def updated_latest_five_hours_traffic_jam(request):
                ...
                for idx, time_value in enumerate(five_hours_list[start_index:end_index], start=start_index):
                    if len(jsonobject["roi_name"]) != 0:
                        for roi_n in jsonobject["roi_name"]:
                            ...
                            TJM_data =TRAFFICJAM_MANAGEMENT_DATA.find_one(variable, {"_id": 0}) #.sort('timestamp', -1))
...
def latest_history(request):
    ...
    if request.method == "GET":
        data = list(TRAFFICJAM_MANAGEMENT_DATA.aggregate([
    {
        "$match": {
        "date": Gettodaydate,
        "analytics_log.status": "Detected"
...
@csrf_exempt
def tj_datewise_history(request):
...
@csrf_exempt
def datewise_hours_traffic_jam(request):
  # result = {'success': False, 'message': 'Something went wrong.'}
  ...
  if request.method == "POST":
    ...
    if not missing_key:
        ...
        if output:
            ...
            TJM_rois = TRAFFICJAM_MANAGEMENT_DATA.distinct(
                    "roi_name",
            ...
            while current_date < to_date:
                for hour in range(24):
                    ...
                    for roi_name in TJM_rois:
                        ...
                        TJM_all_roi_data = list(TRAFFICJAM_MANAGEMENT_DATA.find({
                        ...
                        if TJM_all_roi_data:
                            ...
                            TJM_all_roi_data_1 = list(TRAFFICJAM_MANAGEMENT_DATA.find({
                                "roi_name": roi_name
                            }, {"_id": 0}).sort('timestamp', -1))
...
@csrf_exempt
def trafficjamlivedata( request,camera_name=None,department_name =None):
    ...
    if request.method == 'POST':
        ...
        if not missing_key:
            ...
            if output:
                ...
                data=list(TRAFFICJAM_MANAGEMENT_DATA.aggregate(pipeline))
                if len(data) != 0:
...
@csrf_exempt
def DATEWISERA(request,cameraname=None, pagenumber=None, page_limit=None):
    ret = {'success': False, 'message': 'Something went wrong.'}
    if request.method == 'POST':
        ...
        try:
            ...
            if missing_keys:
                return JsonResponse({'success': False, 'message': f"Missing keys: {', '.join(missing_keys)}"})
            empty_values = [key for key, value in jsonobject.items() if value == '']
            if empty_values:
                return JsonResponse({'success': False, 'message': f"Empty values for keys: {', '.join(empty_values)}"})
            from_date = jsonobject['from_date']
            to_date = jsonobject['to_date']
            department_name = jsonobject['department_name']
...
def TJMIMage(request,roiname,image_file):
    if request.method == 'GET':
        boundingboxdetails =  {
        "object_thickness": 5,
        "roi_thickness": 5,
        ...
        QueryMatch = {"roi_name": roiname, "analytics_log.image_name": image_file}
        image_data = TRAFFICJAM_MANAGEMENT_DATA.find_one(
            QueryMatch,
