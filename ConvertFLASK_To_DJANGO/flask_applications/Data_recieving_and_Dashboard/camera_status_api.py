...
def CrashHelmentviolationCount():
...
def ProtectionZoneviolationCount():
    ret = {'success': False, 'message': 'something went wrong with get CrashHelmentviolationCount api'}    
...
def get_cam_status_enable_cam_count():
    ...
    if 1:
    # try:
        find_data_cam_status = list(mongo.db.ppera_cameras.find({'camera_status': True}))        
...
def get_solution_data_details():
    ...
    # try:
        ...
        wcam_list_and_nwcam_list = get_list_working_not_working_cam_list(list(mongo.db.ppera_cameras.find({'camera_status': True})))
...
def get_current_date_violations():
    ...
    # try:
        ...
        camera_status_true_data = list(mongo.db.ppera_cameras.find({'camera_status': True})) #'camera_rtsp': x_data['rtsp_url']
        # ppepercentage = mongo.db.filterviolations.find_one({},{'_id':0})
        # if ppepercentage is None:
        #     ppepercentage = {"helmet":30,"vest":30}
        ppe_count = 0
        ...
        if len(camera_status_true_data) != 0:
            ...
            ppe_crash_helemt_counts += mongo.db.data.count_documents({'timestamp': {'$regex': '^' + str(date.today())}, 'analyticstype': 'PPE_TYPE2'})
            cr_count += mongo.db.data.count_documents({'timestamp': {'$regex': '^' + str(date.today())},  'analyticstype': 'CRDCNT'}) 
            protected_zone += mongo.db.data.count_documents({
                'timestamp': {'$regex': '^' + str(date.today())},
                'analyticstype': 'RA',
            ...
            ra_count += mongo.db.data.count_documents(match_data_1)
...
def get_not_working_camera_details():
    ret = {'message':'Somthing went wrong.','success':False}
    if 1:
    # try:
        EnabledCameras = list(mongo.db.ppera_cameras.find({'camera_status': True}))
...
def get_all_solns_enable_cam_count():
    ret = {'success': False, 'message':'something went wrong with get all solutions count api'}
    if 1:
    # try:
        require_data_details = []
        """Calling function to get the diable camera list."""
        fun_get_enable_cam_list = NewEnabledcameras(list(mongo.db.ppera_cameras.find({'camera_status': True, 'analytics_status':'true'}))) 
...
def get_enble_ppe_violation_count():
    ret = {'message':'Someting went wrong api','success':False}
    if 1:
    # try:
        find_data_cam_status = mongo.db.ppera_cameras.find({'camera_status': True})
        HELMET_count = 0
        VEST_count = 0
        for find_data in find_data_cam_status:
            ...
            {"object_data.Vest": "no_ppe"}
                                ...
                                data = mongo.db.data.aggregate([{'$match': match_data}, {'$sort': {'timestamp': -1}}]) # , {'$limit': 2}])
                                data = list(data)
                                if len(data) != 0:
                                    ppepercentage = mongo.db.filterviolations.find_one({},{'_id':0})
                                    if ppepercentage  is None:
    ...
    # try:
        ...
        if not missing_key:
            ...
            {"object_data.Vest": "no_ppe"}
                ...
                testing = len(list(mongo.db.data.aggregate([{'$match': match_data},{'$sort': {'timestamp': -1}}])))
                fetch_to_verify_rtsp = mongo.db.data.find_one({'timestamp': { '$regex': '^' + str(date.today())}, 'camera_rtsp': data['camera_rtsp']})
                fetch_to_verify_cam_name = mongo.db.data.find_one({'timestamp':{'$regex': '^' + str(date.today())}, 'camera_name': data['camera_name']})
                if testing != 0:
...
def data_betweem_violations_count():
        ...
        if data != None:
            ...
            if not missing_key:
                ...
                if output:
                    ...
                    from_date = data['from_date']
                    to_date = data['to_date']
                    list_data = list(mongo.db.data.find({'timestamp': {'$gte': from_date, '$lt': to_date}}))
                    if len(list_data) != 0:
                        ppe_count = 0
                        ra_count = 0
                        tc_count = 0
        ...
        if data != None:
            ...
            if not missing_key:
                ...
                if output:
                    ...
                    camera_status_true_data = list(mongo.db.ppera_cameras.find({'camera_status': True}))
                    match_stage = {
                    ...
                    for data in camera_status_true_data:
                        ...
                        list_data = list(mongo.db.data.aggregate([{'$match': match_data}, {'$sort': {'timestamp': -1}}]))
                        ppe_values = data['ppe_data'] #[0] 
                        if len(data['ppe_data']) != 0 and (ppe_values[0]['helmet'] == True or ppe_values[0]['vest'] == True): 
                            ppe_data = list(mongo.db.data.aggregate([{'$match': {'timestamp': {'$gte': from_date,'$lt': to_date}, 'camera_rtsp': data['rtsp_url'], 'analyticstype': 'PPE_TYPE1', '$or': [
            {"object_data.Helmet": False},
            {"object_data.Vest": "no_ppe"}
        ]}}, {'$sort': {'timestamp': -1}}])) 
                        ...
                        if len(data['cr_data']) != 0:
                            cr_count += mongo.db.data.count_documents({'timestamp': {'$gte': from_date,'$lt': to_date}, 'camera_rtsp': data['rtsp_url'], 'analyticstype': 'CRDCNT'}) #list(mongo.db.data.aggregate([{'$match': {'timestamp': {'$regex': '^' + str(date.today())}, 'camera_rtsp': x_data['rtsp_url'], 'analyticstype': 'CRDCNT'}}, {'$sort': {'timestamp': -1}}]))
        ...
        if data != None:
            ...
            if not missing_key:
                ...
                if output:
                    ...
                    from_date = data['from_date']
                    to_date = data['to_date']
                    FOUNDDATA =list(mongo.db.data.find({'timestamp': {'$gte':from_date, '$lt': to_date}}))
                    if len(FOUNDDATA)  !=0:
...
"""CURRENT DATE PPE VIIOLATIONS CAMERA DETAILS"""
@camera_status.route('/cam_wise_PPE_violations_count', methods=['GET'])
def ppe_violations_count_cam_wise():
    ret = {'message': 'something went wrong with ppe_violation coutn cam wise ','success': False}
    if 1:
    # try:
        
        find_data_cam_status = list(mongo.db.ppera_cameras.find({'camera_status': True,'ppe_data':{'$exists':True,"$ne": []}}))
        if len(find_data_cam_status) !=0 :
            ppe_require_data = []
    ...
    # try:
        ...
        if len(find_data_cam_status):
            ...
            for find_data in find_data_cam_status:
                ...
                tc_data = list(mongo.db.trafficcountdata.aggregate([{'$match': match_data}, {'$sort': {'timestamp': -1}}]))
                if len(tc_data) != 0:
...
#             else:
#                 ret['message']= 'No RA Violations Detected' #'there are no RA violations found'
#         else:
#             ret['message']= "No cameras have been added for RA analytics. Please add a camera." #'there are no cameras are added for analytics please add the camera.'
...
def RA_violations_count_cam_wise():
    ...
    find_data_cam_status = mongo.db.ppera_cameras.find({'camera_status': True}, {'cameraname': 1, 'camera_ip': 1, 'rtsp_url': 1, 'imagename': 1, 'roi_data': 1})
    ...
    if camera_rtsp_urls:
        ...
        violation_data = mongo.db.data.aggregate([
            {'$match': match_data},
...
def cam_wise_truck_reversal_RA_violations_count():
        ...
        if len(find_data_cam_status) !=0:
            for find_data in find_data_cam_status:
                ...
                if len(roi_data) != 0:
                    ...
                    ViolationFOUNDDATAfinddata = list(mongo.db.data.aggregate([{'$match': match_data_RA}, {'$sort': {'_id': -1}}]))
                    ...
                    truck_reversal_counts = list(mongo.db.data.aggregate([{'$match': match_data_TRA}]))
...
def CR_violations_count_cam_wise():
    ret = {'message':'Someting went wrong cam_wise_CR_violations_details','success':False}
    if 1:
    # try:
        find_data_cam_status = list(mongo.db.ppera_cameras.find({'camera_status': True, 'cr_data':{'$exists':True,"$ne": []}}))
        if len(find_data_cam_status) !=0:
            cr_require_data_details = []
            for find_data in find_data_cam_status:
                match_data = {'timestamp': {'$regex': '^' + str(date.today())}, 'camera_rtsp': find_data['rtsp_url'], 'analyticstype': 'CRDCNT'}
                FoundCrData = list(mongo.db.data.aggregate([{'$match': match_data}, { '$sort': {'timestamp': -1}}]))
...
def ppe_Crash_helmet_violations_count_cam_wise():
        ...
        if data != None:
            ...
            if not missing_key:
                ...
                if output:
                    ...
                    distinct_ticket_nos = mongo.db.firesmokeviolationdata.distinct('ticket_no', match_data)
...
def fire_violations_count_cam_wise():
    ret = {'message': 'Something went wrong cam_wise_FIRE_violations_details', 'success': False}
    
    find_data_cam_status = mongo.db.ppera_cameras.find({'camera_status': True}, {'cameraname': 1, 'camera_ip': 1, 'rtsp_url': 1, 'imagename': 1, 'firesmoke_data': 1})
    ...
    if camera_rtsp_urls:
        ...
        violation_data = mongo.db.firesmokeviolationdata.aggregate([
            {'$match': match_data},
            {'$group': {'_id': '$camera_rtsp', 'violation_count': {'$sum': 1}}},
            {'$sort': {'_id': 1}}
        ])
...
def SMOKE_violations_count_cam_wise():
...
def DUSTviolationCount():
    ret = {'success': False, 'message': 'something went wrong with get DUSTviolationCount api'}    
    ...
    if request.method == 'POST':
        ...
        ret = {'success': True, 'message': {'dustviolationcount':len(distinct_ticket_nos)}}  
    return ret
...
def DUST_violations_count_cam_wise():
...
def current_date_violations_details():
    ...
    try:
        find_data_cam_status = list(mongo.db.ppera_cameras.find(
            {'camera_status': True},
        ...
        violations = list(mongo.db.data.aggregate(pipeline))
        if not violations:
...
#         #     'rtsp_url': rtsp_url,
#         #     'imagename': find_data['imagename'],
#         #     'ppe_type_counts': mongo.db.data.count_documents({**query_base, 'analyticstype': 'PPE_TYPE1', '$or': [
#         # {"object_data.Helmet": False},
#         # {"object_data.Vest": "no_ppe"}]}),
#         #     'ppe_crash_helemt_counts': mongo.db.data.count_documents({**query_base, 'analyticstype': 'PPE_TYPE2'}),
#         #     'cr_type_counts': mongo.db.data.count_documents({**query_base, 'analyticstype': 'CRDCNT'}),
#         #     'protection_zone_counts': mongo.db.data.count_documents({
#         #         **query_base,
#         #         'analyticstype': 'RA',
#         #         'violation_status': True,
#         #         'object_data': {
...
#         #                                 }
#         #     }),
#         #     'roi_type_counts': mongo.db.data.count_documents({
#         #         **query_base,
#         #         'analyticstype': 'RA',
#         #         'violation_status': True,
#         #         'object_data': {'$elemMatch': {'violation': True, 'roi_details.analytics_type': '0'}}
#         #     })
#         # }
#         # query_base = {
#         #     'timestamp': {'$regex': '^' + str(date.today())},
#         #     'camera_rtsp': find_data['rtsp_url']  # Replace this with actual rtsp_url from find_data
#         # }
...
#         # # Building the required data dictionary and excluding zero counts
#         # required_data = {
#         #     'camera_name': find_data['cameraname'],
#         #     'camera_ip': find_data['camera_ip'],
#         #     'rtsp_url': rtsp_url,
...
#         # # Extract and add non-zero counts to required_data
...
#         (result[0]['roi_type_counts'] and result[0]['roi_type_counts'][0]['count'] > 0):
...
#         # Building the required data dictionary
#         # required_data = {
#         #     'camera_name': find_data['cameraname'],
#         #     'camera_ip': find_data['camera_ip'],
#         #     'rtsp_url': rtsp_url,
#         #     'imagename': find_data['imagename'],
#         #     'ppe_type_counts': ppe_type_counts,
#         #     'ppe_crash_helemt_counts': ppe_crash_helemt_counts,
#         #     'cr_type_counts': cr_type_counts,
#         #     'protection_zone_counts': protection_zone_counts,
...
#         result = {'message': all_violations, 'success': True}
#     else:
#         result= {'message': 'No violations found', 'success':False} # 'there is no violations found'
#     # except Exception as e:
#     #     result['message'] = f'Error occurred: {str(e)}'
#     return result
            ...
            if not missing_key:
                ...
                if output:
                    ...
                    for find_data in camera_status_true_data:
                        ...
                        firesmoke_result = list(mongo.db.firesmokeviolationdata.aggregate(firesmoke_pipeline))
...
#                     # for data in camera_status_true_data:
#                     #     match_data = {'timestamp': {'$gte': from_date,'$lt': to_date}, 'camera_rtsp': data['rtsp_url']}
#                     #     requiredatacameradata={'camera_name': data['cameraname'], 'camera_ip':  data['camera_ip'], 'rtsp_url': data['rtsp_url'], 'imagename':data['imagename'],'ppe_type_counts':0, 'roi_type_counts': 0, 'cr_type_counts':0, 'protection_zone_counts':0, 'ppe_crash_helemt_counts':0}                     
#                     #     if len(data["ppe_data"]) != 0:
#                     #         if len(data['ppe_data']) != 0 and (data['ppe_data'][0]['helmet'] != False or data['ppe_data'][0]['vest'] != False):
#                     #             match_data['analyticstype']='PPE_TYPE1'   
#                     #             get_ppe_counts = fun_ppe_violations_count(list(mongo.db.data.aggregate([{'$match': match_data}, {'$sort': {'timestamp': -1}}])))
#                     #             requiredatacameradata['ppe_type_counts'] = get_ppe_counts["ppe_counts"] #, 'helmet_count':ppe_count["helmet_cnts"], 'vest_count':ppe_count["vest_cnts"]
                       
                        
#                     #         if 'crash_helmet' in data['ppe_data'][0].keys():
#                     #             if data['ppe_data'][0]['crash_helmet'] == True:
#                     #                 ppe_crash_helmet_data = list(mongo.db.data.aggregate([{'$match': {'timestamp': {'$regex': '^' + str(date.today())}, 'camera_rtsp': data['rtsp_url'], 'analyticstype': 'PPE_TYPE2'}}, {'$sort': {'timestamp': -1}}])) 
#                     #                 if len(ppe_crash_helmet_data) != 0:
#                     #                     match_data['analyticstype']='PPE_TYPE2'
#                     #                     ppe_crash_helmet_count = fun_ppe_crash_helmet_violations_count(ppe_crash_helmet_data) # list(mongo.db.data.aggregate([{'$match': match_data}, {'$sort': {'timestamp': -1}}])))          
#                     #                     requiredatacameradata['ppe_crash_helemt_counts'] = ppe_crash_helmet_count["crash_helmet_cnts"]
                                        
#                     #     if len(data['roi_data']) != 0:
#                     #         #match_data['analyticstype']='RA'
#                     #         match_data_NRA = {'timestamp':{'$gte': from_date,'$lt': to_date},'camera_rtsp': data['rtsp_url'],
#                     #           'analyticstype': 'RA',
#                     #           'violation_status': True,
#                     #           'object_data': {
#                     #                     '$elemMatch': {
...
#                     #                  }
#                     #         }
#                     #         requiredatacameradata['roi_type_counts'] = len(list(mongo.db.data.aggregate([{'$match': match_data_NRA}, {'$sort': {'timestamp': -1}}])))  

#                     #         match_dat_TRA = {'timestamp':{'$gte': from_date,'$lt': to_date},'camera_rtsp': data['rtsp_url'],
#                     #           'analyticstype': 'RA',
#                     #           'violation_status': True,
#                     #           'object_data': {
#                     #                             '$elemMatch': {
...
#                     #                         }
#                     #         }
#                     #         requiredatacameradata['protection_zone_counts'] = len(list(mongo.db.data.aggregate([{'$match': match_dat_TRA}, {'$sort': {'timestamp': -1}}])))  

#                     #     if len(data['cr_data']) != 0:
#                     #         match_data['analyticstype']='CRDCNT'
#                     #         requiredatacameradata['cr_type_counts'] = len(list(mongo.db.data.aggregate([{'$match': match_data}, {'$sort': {'timestamp': -1}}])))
                            
#                     #     all_violations.append(requiredatacameradata) 
        ...
        if find_data_cam_status:
            ...
            for find_data in find_data_cam_status:
                ...
                if roi_data:
                    ...
                    violation_data = mongo.db.data.count_documents(match_data)
...
def date_wise_CR_violations_cam_details():
...
def date_wise_ppe_crash_helmet_violations_details():
    ret = {'message': 'something went wrong in date_wise_PPE_violations_details API.','success': False}
    if 1:
        data = request.json
        request_key_array = ['from_date', 'to_date']
...
def cam_wise_truck_reversal_RA_violations_by_date():
    ret = {'message': 'something went wrong in date_wise_RA_violations_details API.','success': False}
    if 1:
        data = request.json
        request_key_array = ['from_date', 'to_date']
        if data != None:
            ...
            if not missing_key:
                ...
                if output:
                    ...
                    find_data_cam_status = list(mongo.db.ppera_cameras.find({'camera_status': True, 'roi_data':{'$exists':True,"$ne": []}}))
                    if len(find_data_cam_status) !=0:
                        roi_require_data_details = []
                        for find_data in find_data_cam_status:
                            """Calling function to get the working camera list and not woorking camera list"""
