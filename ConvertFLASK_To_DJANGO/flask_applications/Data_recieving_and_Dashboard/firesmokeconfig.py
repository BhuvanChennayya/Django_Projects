...
firesmoke = Blueprint('firesmoke', __name__)
...
def FIREANDSMOKEDUSTIMAGE(imagename):
    #Fsd = (127, 255, 0)
    boundingboxdetails =  {
     "object_thickness": 5,
    "roi_thickness": 5,
    ...
    # try:
        ...
        image_data = mongo.db.firesmokeviolationdata.find_one({'analytics_details.details.image_name':imagename})#({'video_file_name': imagename})
...
def DeleteFIREviolation(id=None):
...
def FIRESMOKEVERIFICATIONviolation(id=None, flag=None):
    ret = {'message': 'something went wrong with violation status .','success': False}
    try:
        if id is not None:
            if flag is not None:
...
def FSDLiveviolationdata( camera_name=None):
    ...
    if 1:    
        ...
        if camera_name is not None :
            ...
            data = list(mongo.db.firesmokeviolationdata.aggregate([{'$match': match_data},{'$group':{'_id':{'ticket_no':'$ticket_no'}, 'data':{'$push':'$$ROOT'}}},{'$limit': 4000000}, {'$sort':{'data._id': -1}} ]))
            if len(data) != 0:
            ...
            data = list(mongo.db.firesmokeviolationdata.aggregate([{'$match': match_data},{'$group':{'_id':{'ticket_no':'$ticket_no'}, 'data':{'$push':'$$ROOT'}}},                                                  
...
def datewiseFiresmoke(cameraname=None, pagenumber=None, page_limit=None):
    ...
    if request.method == 'POST':
        ...
        if not missing_key:
            ...
            if output:
                ...
                to_date = jsonobject['to_date']
                match_data = {'timestamp':{'$gte':from_date, '$lte': to_date}}
                data = list(mongo.db.firesmokeviolationdata.aggregate([{'$sort': {'timestamp': -1}},{'$match': match_data},{'$group': {'_id': {'camera_name': '$camera_name'}, 'data': {'$first': '$$ROOT'}}},
                                                    {'$project': {'data': 0}}]))
                dash_data = []
                if len(data) != 0:
                    for count, i in enumerate(data):
...
def create_violation_excelFireSmoke():
    ret = {'success': False, 'message':'Something went wrong, please try again later'}
    if 1:
    # try:
        if not os.path.exists('firesmoke_violation_excel_sheets'):
...
#             end_col = x + width
#             end_row = y + height
#             draw.rectangle([start_col, start_row, end_col, end_row], outline=(127, 255, 0), width=7)
#         imagename = os.path.splitext(image_path)[0]
#         img.save(os.path.join(get_current_dir_and_goto_parent_dir(), 'images', 'frame', imagename+'_1.jpg'))
#         worksheet.insert_image(row, column, os.path.join(get_current_dir_and_goto_parent_dir(), 'images', 'frame', imagename+'_1.jpg'), {'x_scale': 0.16, 'y_scale': 0.212})
...
#Text is writing above 
# def draw_bbox_and_insert_image(worksheet, image_path, image_data, row, column):
#     img = Image.open(os.path.join(get_current_dir_and_goto_parent_dir(), 'images', 'frame', image_path))
#     draw = ImageDraw.Draw(img)
    
#     # Load the font for the class name label
#     font = ImageFont.truetype('/usr/share/fonts/truetype/freefont/FreeMonoBold.ttf', 40, encoding='unic')
...
#             # Draw bounding box
#             draw.rectangle([start_col, start_row, end_col, end_row], outline=(127, 255, 0), width=6)

#             # Calculate text size
#             text_width, text_height = calculate_text_size(class_name, 40)
#             text_x = start_col + 6
#             text_y = start_row - text_height - 10  # Position above the bounding box
...
#             draw.text((text_x, text_y), class_name, fill=(127, 255, 0), font=font)
...
#             text_width, text_height = calculate_text_size(class_name, 40)
#             text_x = start_col + 6
#             text_y = end_row + 10  # Position below the bounding box
#             text_bg_position = (text_x, text_y, text_x + text_width + 10, text_y + text_height)
#             draw.rectangle(text_bg_position, fill=(0, 0, 0, 128))
#             draw.text((text_x, text_y), class_name, fill=(127, 255, 0), font=font)
#         imagename = os.path.splitext(image_path)[0]
...
def firesmokeviolation_excel_download():
    if 1:
    # try:
