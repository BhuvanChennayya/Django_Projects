...
parking_manage_data = Blueprint('parking_manage_data', __name__)
...
def get_latest_five_hours_data():
    ...
    if not missing_key:
        ...
        if output:
            ...
            for idx, time_value in enumerate(five_hours_list[start_index:end_index], start=start_index):
                ...
                # VPMS_data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.find({"timestamp":{'$gte':five_hours_list[idx], '$lte': five_hours_list[idx+1]}}, {"_id":0, "dashboard_log":0}).sort('timestamp', -1))
                VPMS_data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.find({"timestamp":{'$gte':five_hours_list[idx], '$lte': five_hours_list[idx+1]}, "camera_name":jsonobject["camera_name"]}, {"_id":0, "dashboard_log":0}).sort('timestamp', -1))
            ...
            result = {"PA_message":five_hours_list_PA_data, "NPA_message":five_hours_list_NPA_data,"success":True}
    return result


@parking_manage_data.route('/get_latest_five_hours_PA_NPA_data', methods=['GET'])
def get_latest_five_hours_PA_NPA_data():
    result = {"message":"Something went wrong with 'get_latest_five_hours_data'", "success":False}
    five_hours_list = get_five_hours_list()
    five_hours_list_PA_data = []
    five_hours_list_NPA_data = []
    ...
    for idx, time_value in enumerate(five_hours_list[start_index:end_index], start=start_index):
        ...
        VPMS_data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.find({'date':date,"timestamp":{'$gte':five_hours_list[idx], '$lte': five_hours_list[idx+1]}}, {"_id":0, "dashboard_log":0}).sort('timestamp', -1))

         # print("VPMS DATA:----dataaaaaaaaaaaa---------------", VPMS_data) # idx, five_hours_list[idx], five_hours_list[idx+1], len(VPMS_data))
        TotalSlotsOFLastFiveHours = mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate([
        ...
        if len(VPMS_data) !=0 :
            ...
            #     }}
            # ]
            TotalNPAViolationAcount = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate(pipeline))
            if len(TotalNPAViolationAcount) !=0 :
                ...
                parkedcount = mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.count_documents(query)
...
def slots_status_details(val=None):
    ...
    if val is not None and val in ['PA', 'NPA']:
        ...
        result = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate(pipeline))
        ...
        count_pa_parked = mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.count_documents(query_pa_parked)
        count_pa_free = mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.count_documents(query_pa_free)
        count_npa_parked = mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.count_documents(query_npa_parked)
        count_npa_free = mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.count_documents(query_npa_free)
        ...
        result_pa_violation_1 = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate(pipeline_pa_violation_1))
        result_pa_violation_0 = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate(pipeline_pa_violation_0))
        result_npa_violation_1 = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate(pipeline_npa_violation_1))
        result_npa_violation_0 = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate(pipeline_npa_violation_0))
...
def get_cameralist():
    ...
    VPMS_data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.find({"date":formatted_date},{'_id':0, 'dashboard_log':0}).sort('timestamp', -1))
...
def all_camera_slot_details():
    ...
    results = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate(pipeline))
    VPMS_data = results
...
@parking_manage_data.route('/slot_details', methods=['POST'])#methods=['GET',"POST"]
@parking_manage_data.route('/slot_details/<val>', methods=['GET'])
# @parking_manage_data.route('/slot_details/<val>', methods=['GET'])
def slot_details(val=None):
    ...
    if request.method == 'POST':
        ...
        if not missing_key:
            ...
            if output:
                ...
                VPMS_data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.find(MainQuery, {"_id":0, "dashboard_log":0}).sort('timestamp', -1))
                if len(VPMS_data) != 0:
                    ...
                    PAVPMS_data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate(PApipeline))
                    ...
                    NPAVPMS_data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate(pipeline))
                    cameraname_list = []
            ...
            for PA_data in VPMS_data:
                ...
                for analytics_data in PA_data["analytics_log"]:
                    ...
                    # elif NPA != None:
                        ...
                        total_NPA_slots_count = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.find({"date":formatted_date, "camera_name":PA_data["camera_name"]}, {"_id":0, "dashboard_log":0}).sort('timestamp', -1))
                        for analytics_data in PA_data["analytics_log"]:
                            # violation_count = 0
                            # valid_parked_count = 0
                            # print("ANALYTICS DATA:-------------------", analytics_data["details"])
...
def get_VPMS_history():
    today = datetime.today()
    formatted_date = today.strftime('%Y%m%d')
    all_data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.find(
...
def PA_Slots():
    ...
    if not missing_key:
        ...
        if output:
            ...
            print("jsonobject:-----------------", jsonobject)
            all_slots= list(mongo.db.VPMS_DATA.find({"camera_rtsp":jsonobject["camera_name"], "date":formatted_date},{'analytics_log':0,'date':0, 'id':0, 'dashboard_log':0, '_id':0, 'camera_rtsp':0, 'analytics_type':0}).sort('date', -1))
...
def latest_history():
    ...
    formatted_date = today.strftime('%Y%m%d')
    match_data = {"date":formatted_date}
    data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.aggregate([{'$match': match_data}, {'$limit': 4000000}, {'$sort':{'timestamp': -1}}, {'$limit': 1},
...
def datewise_camera_list():
    ...
    if not missing_key:
        ...
        if output:
            ...
            data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.find({'timestamp':{'$gte':from_date, '$lte': to_date}},
    ...
    try:
        ...
        if not missing_key:
            ...
            if output:
                ...
                print("--------------------------mainQuery-----------------",mainQuery)
                violation_data = list(mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.find(mainQuery
...
def datewise_violation_excel(from_date, to_date):
    ret = {'success': False, 'message': 'Something went wrong.'}
    try:
        mainQuery= {}
        jsonobject = request.json
...
def create_violation_excelVPMS():
    ...
    mongo_data = datewise_violation_excel(from_date, to_date)
...
def VPMS_violation_excel_download():
    if 1:
    # try:
...
def get_bbox_image(image_file,roiname =None):
    ...
    image_data= mongo.db.VEHICLE_PARKING_MANAGEMENT_DATA.find_one(
    QueryMatch,
    sort=[('_id', -1)])
...
def get_img_bbox(image_file,roiname =None):
    boundingboxdetails =  {
     "object_thickness": 5,
    "roi_thickness": 5,
    "object_font_size": 12,
...
def NPAimage(image_file,roiname =None):
    boundingboxdetails =  {
     "object_thickness": 5,
    "roi_thickness": 5,
    "object_font_size": 12,
