...
proofofconcept = Blueprint('proofofconcept', __name__)




@proofofconcept.route('/add_wheelcount_data', methods=['POST'])
def add_wheelcount_data():
                                                ...
                                                if check_dictionaryishavinganynonevalue(fetched_tc_data['line_bbox']):
                                                    if 'wheel_count' in finddata['poc']:
                                                        if check_the_tc_data_is_key_idisexist(finddata['poc']['wheel_count'],fetched_tc_data['roi_id']):
                                                            ...
                                                            if ai_solutions is not None:
                                                                if isEmpty(ai_solutions):
                                                                    ...
                                                                    result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'poc.wheel_count': returntcdata,'ai_solution': finddata['ai_solution']}}))
                                                        ...
                                                        if ai_solutions is not None:
                                                            if isEmpty(ai_solutions):
                                                                finddata['ai_solution'].update(ai_solutions)
                                                                result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'poc.wheel_count': tc_data,'ai_solution': finddata['ai_solution']}}))
                                                                if result.modified_count > 0:
                                                                    ret = {'message': 'wheel count data added successfully.', 'success': True}
                                                                else:
                                                                    ret['message'] = 'wheel count data not adeed.'
                                ...
                                for __, i in enumerate(fetch_tc_data):
                                    if i['roi_id']==roi_id:
                                        ...
                                        if i not in update_data:
                                            update_data.append(i)
                                result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'poc.wheel_count': update_data,'ai_solution': finddata['ai_solution']}}))
                                if result.modified_count > 0:
                            ...
                            if 'wheel_count' in tc_data:
                                ...
                                if isEmpty(tc_data) :
                                    ...
                                    if len(tc_data['wheel_count']) == 1:
                                        finddata['ai_solution'].update(ai_solutions)
                                        result = mongo.db.ppera_cameras.update_one( {'_id': ObjectId(id)}, {'$set': {'poc.wheel_count': [], 'ai_solution':finddata['ai_solution']}})
                                        if result.modified_count > 0:
...
def Getwheelcountdata():
    ...
    fetch_require_data = list(mongo.db.ppera_cameras.find({'camera_status': True, "analytics_status": 'true','poc.wheel_count':{'$exists':True,"$ne": []}}))
...
def wheelcountdumpvoiceannaoumentdataintodatatable(getdata_response):
    if "voice_announcement_status" not in mongo.db.list_collection_names():
        ...
        mongo.db.voice_announcement_status.delete_many({"violation_type": { "$in": ["wheelcount"] }})
    for i , j in enumerate(getdata_response):
        ...
        if len(j['poc']) !=0:
            ...
            if len(insertvoice_datawheelcount) !=0:
                mongo.db.voice_announcement_status.insert_many(insertvoice_datawheelcount)
...
def LIVEwheelcount(camera_name=None,department_name=None):
    ...
    if request.method == 'POST':
        ...
        if not missing_key:
            ...
            data = list(mongo.db.wheelcount.aggregate(pipeline))
        ...
        if len(data) != 0:
            ...
            match_data = {'timestamp':{'$regex': '^' + str(date.today())}}
            data = list(mongo.db.wheelcount.aggregate([
                                        {'$match': match_data},
                                        {'$group': {'_id': {'camera_name': '$camera_name'}, 'data': {'$push': '$$ROOT'}}},
                                        {'$limit': 4000000},
                                        {'$sort': {'timestamp': -1}},
...
def wheelrotationcount(live_data_count, all_data):
    try:
        data = mongo.db.wheelrotationcount.find_one()
        live_data_30 = []
        if data is not None:
            ...
            result = mongo.db.wheelrotationcount.update_one({'_id': ObjectId(data['_id'])}, {'$set':{'live_data_count': data['live_data_count'],'page_limit': data['page_limit'], 'page_num': data['page_num']}})
            ...
            result = mongo.db.wheelrotationcount.insert_one(dictionary)
...
def testwheelcount(camera_name=None,department_name=None):
    ret = {'success': False,'message':"something went wrong in TrafficCountLiveCount apis"}
    match_data = {'timestamp':{'$regex': '^' + str(date.today())}}
    pipeline=[]
    if request.method == 'POST':
...
def wheelroationlivedata( camera_name=None):
    ...
    if 1:    
        ...
        if camera_name is not None :
            ...
            data = list(mongo.db.wheelrotation.aggregate([{'$match': match_data},{'$limit': 4000000}, {'$sort':{'timestamp': -1}} ]))
            if len(data) != 0:
            ...
            match_data = {'timestamp':{'$regex': '^' + str(date.today())}}
            data = list(mongo.db.wheelrotation.aggregate([{'$match': match_data},                                                  
...
def wheelroationimage(imagename):
    if 1:
    # try:
        base_path = get_current_dir_and_goto_parent_dir()+'/images/frame'
        file_path = os.path.join(base_path, imagename)
        image_data = mongo.db.wheelrotation.find_one({'object_details.details.image_name':imagename})#({'video_file_name': imagename})
