...
check_camera = Blueprint('check_camera', __name__)
...
def write_excel(cam_status_file):
    ...
    sh = wb.active
    #Get latest data
    trial = mongo.db.camera_excel_details.find_one({},sort=[( '_id', pymongo.DESCENDING )])
    row_list = list(mongo.db.excel_cameras.find({"token": trial["token"]}))
    if len(row_list) != 0:
        ...
        if len(verify_the_keys) != 0:
            ...
            if camera_ip_key in list_keys and rtsp_key in list_keys:
                for data_1 in require_data:
                    ...
                    elif "ip_address_not_given" in data_1.values() and "rtsp_working" in data.values():
                        for col, value in data_1.items():
                            sh.cell(row=2, column=col, value=value)
                            sh.cell(row=2, column=col).fill = PatternFill(start_color="66ff99", end_color="66ff99",fill_type = "solid")
                            sh.cell(row=2, column=col).alignment = Alignment(horizontal='center', vertical='center')
            ...
            for cam_data in all_json_data:
                ...
                if camera_ip_key in list_parameters_ls and rtsp_key not in list_parameters_ls:
                    ...
                    if cam_data[camera_ip_key] != None:
                        ...
                        # insert_cam_ip_data = mongo.db.excel_cameras.find_one({camera_ip_key: cam_data[camera_ip_key], "ip_status": res})
                        insert_cam_ip_data = mongo.db.excel_cameras.find({camera_ip_key: cam_data[camera_ip_key], "ip_status": res, "token": unique_str})

                        if len(list(insert_cam_ip_data)) == 0:
                            mongo.db.excel_cameras.insert_one(cam_data)
                ...
                #only RTSP is avilable in the excel sheet.
                    ...
                    if cam_data[rtsp_key] != None:
                        ...
                        insert_rtsp_data = mongo.db.excel_cameras.find_one({rtsp_key: cam_data[rtsp_key], "rtsp_status": res_rtsp, "token": unique_str})
                ...
                # Both CAMERA IP and RTSP are in the excel sheet.
                    ...
                    if cam_data[rtsp_key] != None and cam_data[camera_ip_key] != None:
                        ...
                        insert_data = mongo.db.excel_cameras.find_one({camera_ip_key: cam_data[camera_ip_key], "ip_status": res, "rtsp_status":res_rtsp, "token": unique_str})
                        if insert_data == None:
                            mongo.db.excel_cameras.insert_one(cam_data)
                    ...
                    #Both RTSP and Cam_ip not given
                        ...
                        insert_data = mongo.db.excel_cameras.find_one({camera_ip_key: cam_data[camera_ip_key], "ip_status": "ip_address_not_given"})
                        if insert_data == None:
                            mongo.db.excel_cameras.insert_one(cam_data)
                    ...
                    #RTSP is empty and Cam_ip had given      
                        ...
                        res = ping_test(cam_data[camera_ip_key])
                        cam_data.update({"ip_status": res, "rtsp_status": "rtsp_not_given", "token": unique_str})
                        insert_data = mongo.db.excel_cameras.find_one({camera_ip_key: cam_data[camera_ip_key], "ip_status": res, "token": unique_str})
                        if insert_data == None:
                            mongo.db.excel_cameras.insert_one(cam_data)
                    ...
                    # """Cam_ip is empty and RTSP had given."""        
                        ...
                        res_rtsp = CHECKRTSPWORKINGORNOT(rtsp)
                        cam_data.update({"ip_status": "ip_address_not_given", "rtsp_status": res_rtsp, "token": unique_str})
                        insert_data = mongo.db.excel_cameras.find_one({camera_ip_key: cam_data[camera_ip_key], "rtsp_status": res_rtsp, "token": unique_str})
                        if insert_data == None:
                            mongo.db.excel_cameras.insert_one(cam_data)
...
def upload_cameras_excel():
    ...
    try:
        ...
        if res == True:
            ...
            file.save(filename_db)

            #Fun to generate a token.
            token = GENERATEALPHANUMERICKEYFOREXCELTEST50()
            
            #Verifying the inserting data is exist or not in the db.
            verify_data = mongo.db.camera_excel_details.find_one({"requested_time": current_timestamp, "token": token})
            if verify_data == None:                                             
                #If not data not exist insert to the db.
                mongo.db.camera_excel_details.insert_one({"requested_time":current_timestamp, "token": token}) 
