...
camera_coin = Blueprint('camera_coin', __name__)
            ...
            for index, eachrow in enumerate(ALL_DATA):
                ...
                if eachrow['ip_status']['status'] :                        
                    ...
                    if eachrow['ip_status']['status'] == True:
                        find_data = None#mongo.db.ppera_cameras.find_one({ 'timestamp':{'$regex': '^' + str(date.today())}})#'no': eachrow['no'],
                        if find_data is None:
                            ...
                            if eachrow['data'] is not None:
                                ...
                                if isEmpty(eachrow['data']) :
                                    ...
                                    if 'ip_address'in eachrow:
                                        del eachrow['ip_address']
                                    result = mongo.db.ppera_cameras.insert_one(eachrow)
...
#REPLACE IT IN CAMERA_COIN_APIS.py file 
# def checkdictionarfortruevaluesonly(dictionary,dataobject):
#     print("dictionary===data==",dictionary)
#     print("--------------dataobject---------",dataobject)
#     updatestatus = False
...
#                     status_val = 0
#                     for x in dataobject['roi_data']:
#                         if x['analyticstype'] == 0: # in x['analyticstype'].values():
#                             status_val += 0
#                             break
#                         # if x['analyticstype'] == 2:
#                     if status_val > 0:
...
#                     status_val = 0  
#                     for x in dataobject['roi_data']:
#                         if x['analyticstype'] == 2: # in x['analyticstype'].values():
#                             status_val += 1
#                             break
#                         # if x['analyticstype'] == 2:
#                     if status_val > 0:
        ...
        if check1:
            ...
            if output:
                if len(output) ==1 and 'genetic_id' in output:
                    finddata = mongo.db.ppera_cameras.find_one({'_id': ObjectId(data['id'])})
                            ...
                            if (finddata['ai_solution']==data['ai_solution']) != True:
                                ...
                                if has_true_value:    
                                    ...
                                    if checkdictionarfortruevaluesonly(returnvalue,finddata)  : 
                                        ...
                                        result = mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': data})
    ...
    if isEmpty(data):
        ...
        if check is True:
            ...
            if output:
                ...
                finddata = mongo.db.ppera_cameras.find_one({'_id': ObjectId(data['id'])})
                if finddata is not None: 
                    result = mongo.db.ppera_cameras.update_one({'_id': ObjectId(data['id'])}, {'$set': data})
                    if result.modified_count > 0:
                            ...
                            if roi_data == finddata['roi_data']:
                                    ...
                                    if ai_solutions is not None:
                                        if type(ai_solutions) == list:
                                            ...
                                            if isEmpty(ai_solutions) :
                                                ...
                                                print("ai_solutions")
                                                result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'ai_solution': finddata['ai_solution'],'ppe_data': ppe_data}}))
                                                if result.modified_count > 0:
                                                    ret = {'message':'ppe added successfully.', 'success': True}
                                                else:
                                                    ret['message'] = 'ppe not adeed.'
                                ...
                                if ai_solutions is not None:
                                    if type(ai_solutions) == list:
                                        ...
                                        if isEmpty(ai_solutions) :
                                            finddata['ai_solution'].update(ai_solutions)
                                            result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'roi_data': roi_data,'ai_solution': finddata['ai_solution'],'ppe_data': ppe_data}}))
                                            if result.modified_count > 0:
                                                ret = {'message': 'roi added successfully.','success': True}
                                            else:
                                                ret['message'] = 'roi not adeed.'
                        ...
                        if isEmpty(ai_solutions):
                            ...
                            if len(fetch_roi_data) != 0:
                                if len(fetch_roi_data) == 1:
                                    finddata['ai_solution'].update(ai_solutions)
                                    result = mongo.db.ppera_cameras.update_one( {'_id': ObjectId(id)}, {'$set': {'roi_data': roi_data, 'ai_solution':finddata['ai_solution']}})
                                    if result.modified_count > 0:
                                    ...
                                    if len(roi_data) == 1:
                                        ...
                                        result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'roi_data': update_data,'ai_solution': finddata['ai_solution']}}))
                                        if result.modified_count > 0:
                        ...
                        if isEmpty(ai_solutions) :
                            ...
                            if len(roi_data) != 0:
                                ...
                                if len(roi_data) == 1:
                                    ...
                                    result = mongo.db.ppera_cameras.update_one( {'_id': ObjectId(id)}, {'$set': {'roi_data': [], 'ai_solution':finddata['ai_solution']}})
                                    if result.modified_count > 0:
                                            ...
                                            if cr_data[0]['bb_box'] ==''and cr_data[0]['area_name']=='':
                                                ...
                                                if check_arraydictionaryishavinganynonevalue(cr_data[0]['data_object']):
                                                    if check_dataobjects_of_cr_data(cr_data[0]['data_object']):
                                                        finddata['ai_solution'].update(ai_solutions)
                                                        result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'cr_data':cr_data,'ai_solution': finddata['ai_solution']}}))
                                                        # print()
                                                                ...
                                                                if len(finddata['cr_data']) !=0:
                                                                    ...
                                                                    if check_the_tc_data_is_key_idisexistCRdata(finddata,cr_data[0]['roi_id']):
                                                                        ...
                                                                        if len(final_cr_data) != 0 :
                                                                            ...
                                                                            finddata['ai_solution'].update(ai_solutions)
                                                                            result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)},{'$set': {'cr_data':final_cr_data,'ai_solution': finddata['ai_solution']}}))
                                                                            if result.modified_count > 0:
                                                                                ret = {'message': 'cr data added successfully.','success': True}
                                                                            else:
                                                                                ret['message'] = 'cr data not adeed.'
                                    ...
                                    if len(cr_data) == 1:
                                        ...
                                        for __, i in enumerate(fetch_cr_data):
                                            if int(i['roi_id']) == int(cr_data[0][ 'roi_id']):
                                                ...
                                                update_data.append(i)
                                        result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'cr_data': update_data,'ai_solution': finddata['ai_solution']}}))
                                        if result.modified_count > 0:
                                            ret = {'message': 'cr_data data updated successfully.','success': True}
                                        else:
                                            ret['message'] = 'cr_data not updated.'
                    ...
                    if roi_id is not None:
                        if isEmpty(ai_solutions):
                            ...
                            if len(cr_data) != 0:
                                ...
                                if len(cr_data) == 1:
                                    finddata['ai_solution'].update(ai_solutions)
                                    result = mongo.db.ppera_cameras.update_one( {'_id': ObjectId(id)}, {'$set': {'cr_data': [], 'ai_solution':finddata['ai_solution']}})
                                    if result.modified_count > 0:
                                            ...
                                            if check_dictionaryishavinganynonevalue(fetched_tc_data['line_bbox']):
                                                if check_the_tc_data_is_key_idisexist(finddata['tc_data'],fetched_tc_data['roi_id']):
                                                    ...
                                                    if ai_solutions is not None:
                                                        if isEmpty(ai_solutions):
                                                            finddata['ai_solution'].update(ai_solutions)
                                                            result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'ai_solution': finddata['ai_solution'],'tc_data': returntcdata}}))
                                                            if result.modified_count > 0:
                        ...
                        if isEmpty(ai_solutions):
                            ...
                            if len(fetch_tc_data) != 0:
                                if len(fetch_tc_data) == 1:
                                    finddata['ai_solution'].update(ai_solutions)
                                    result = mongo.db.ppera_cameras.update_one( {'_id': ObjectId(id)}, {'$set': {'tc_data': tc_data, 'ai_solution':finddata['ai_solution']}})
                                    if result.modified_count > 0:
                                    ...
                                    if len(tc_data) == 1:
                                        ...
                                        for __, i in enumerate(fetch_tc_data):
                                            if int(i['roi_id']) == int(tc_data[0][ 'roi_id']):
                                                ...
                                                update_data.append(i)
                                        result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'tc_data': update_data,'ai_solution': finddata['ai_solution']}}))
                                        if result.modified_count > 0:
                                            ret = {'message': 'tc_data data updated successfully.','success': True}
                                        else:
                                            ret['message'] = 'tc_data not updated.'
                    ...
                    if roi_id is not None:
                        if isEmpty(ai_solutions):
                            ...
                            if len(tc_data) != 0:
                                ...
                                if len(tc_data) == 1:
                                    finddata['ai_solution'].update(ai_solutions)
                                    result = mongo.db.ppera_cameras.update_one( {'_id': ObjectId(id)}, {'$set': {'tc_data': [], 'ai_solution':finddata['ai_solution']}})
                                    if result.modified_count > 0:
...
def delete_multipl_camera():
    ret = {'success': False, 'message':'something went wrong with delete ra camera api'}
    if request.method == 'GET':
        ret = {'success': False, 'message':'something went wrong with delete ra camera api'}
        try:
            find_data = mongo.db.ppera_cameras.find_one({})
            if find_data is not None:
                delete_data = mongo.db.ppera_cameras.delete_many({})
                if delete_data.deleted_count > 0:
                    #delete_hooter_data(find_data)
                    ret = {'message': 'camera is deleted successfully.','success': True}
                else:ret['message' ] = 'data is not deleted, please try once again.'
...
#     ret = {"message":"record queue is max limit for this camera","success":False}
#     data_found_for_coin = mongo.db.record_log.find_one({'timestamp':{'$regex': '^' + str(date.today())},"camera_ip":data['camera_ip'],'cameraname':data['cameraname'],'camera_rtsp':data['camera_rtsp'],'duration':data['duration'],'coinid':data['coinid'],"cameraid":data['cameraid'],'record_status': 0})
#     if data_found_for_coin is not None:
#         print("data not found for coin")
#         filters ={'_id': ObjectId(data_found_for_coin['_id'])} #{'analytics_status': status}
#         newvalues = {'$set':{'record_status': 1}}
#         result = mongo.db.record_log.update_one(filters, newvalues)
#         if result.modified_count > 0 :
#             ret= {'message':"violation data added to queue successfully",'success':True}
#         else:
#             ret['message'] = 'data is not inserted properly, please try once again.'
    ...
    # try:
        ...
        if not missing_key:
            ...
            if output:
                ...
                # finddata = mongo.db.ppera_cameras.find_one({'_id': ObjectId(id)})
                finddata = mongo.db.ppera_cameras.find_one({"coin_details":{"$elemMatch":{"coin_id": coin_id}}})
                if finddata is not None:
                    ...
                    coinid_data = mongo.db.coin_id_violation_data.find_one({'timestamp':{'$regex': '^' + str(date.today())},"camera_ip":camera_ip,'cameraname':cameraname,'camera_rtsp':camera_rtsp,'duration':data['recording_time'],'coinid':data['coin_id'],"cameraid":cameraid,'datauploadstatus':{"＄ne": 1}})
                                            ...
                                            if rotation_return['success'] :
                                                ...
                                                if insertion_response['error_status'] == False:
                                                    if insertion_response['success']:
                                                        result = mongo.db.coin_id_violation_data.insert_one(insertion_data)
                                                        if result.acknowledged:
...
def CoinIDViolationData():
    ret= {"message":"something went wrong with get_coin_violationDATA","success":False}
    if request.method == 'GET':
        { "status": { "$in": [None,1 ] }}
        # { "smartrecordkeyid": { "$in": [None, ] }}
        match_data = {'timestamp':{'$regex': '^' + str(datetime.date.today())},"status": { "$in": [None,1 ] }}
        find_data = list(mongo.db.coin_id_violation_data.find(match_data))
        if len(find_data) != 0:
            count1 = 1 
            dashdata =[]
...
def COinidlist():
    ...
    data = list(mongo.db.ppera_cameras.aggregate([{'$match': match_data},{'$group':{'_id':{'coindata': '$coin_details.coin_id'}, 'all_data':{'$first': '$$ROOT'}}},{'$limit': 4000000}]))
    dash_data = []
    if len(data) != 0:
...
def getcoinidcameralist():
    ret = {"message":"something went wrong with getcoinidlist api ","success":False}
    match_data = {"alarm_type":"sensegiz","camera_status":True,"coin_details": { "$exists": True, "$not": {"$size": 0} } }
    data = list(mongo.db.ppera_cameras.aggregate([{'$match': match_data},{'$group':{'_id':{'cameraname': '$cameraname'}, 'all_data':{'$first': '$$ROOT'}}},{'$limit': 4000000}]))
    dash_data = []
    if len(data) != 0:
        for count, i in enumerate(data):
            if len(i['all_data']) !=0:
                            ...
                            if "firesmoke_data" in finddata:
                                if firesmoke_data == finddata['firesmoke_data']:
                                    ...
                                    if ai_solutions is not None:
                                        if isEmpty(ai_solutions) :
                                            finddata['ai_solution'].update(ai_solutions)
                                            result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'firesmoke_data': firesmoke_data,'ai_solution': finddata['ai_solution']}}))
                                            if result.modified_count > 0:
                            ...
                            if "firesmoke_data" in finddata:
                                if firesmoke_data == finddata['firesmoke_data']:
                                    ...
                                    if ai_solutions is not None:
                                        if isEmpty(ai_solutions) :
                                            finddata['ai_solution'].update(ai_solutions)
                                            result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'firesmoke_data.roi_parameters': firesmoke_data['roi_parameters'],'ai_solution': finddata['ai_solution']}}))
                                            if result.modified_count > 0:
                                                ret = {'message': 'firesmoke_data added successfully.','success': True}
                                            else:
                                                ret['message'] = 'firesmoke_data not adeed.'
                        ...
                        if len(firesmoke_data) != 0:
                            if "firesmoke_data" in finddata:
                                if firesmoke_data == finddata['firesmoke_data']:
                                    ...
                                    result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'firesmoke_data.static_parameters': firesmoke_data}}))
                                    if result.modified_count > 0:
                        ...
                        if isEmpty(fsddata) :
                            ...
                            if 'roi_parameters' in fsddata:
                                ...
                                if len(fsddata['roi_parameters']) != 0:
                                    ...
                                    finddata['ai_solution'].update(ai_solutions)
                                    result = mongo.db.ppera_cameras.update_one( {'_id': ObjectId(id)}, {'$set': {'firesmoke_data.roi_parameters':update_data, 'ai_solution':finddata['ai_solution']}})
...
#                             if len(fsddata) == 1:
#                                 finddata['ai_solution'].update(ai_solutions)
#                                 result = mongo.db.ppera_cameras.update_one( {'_id': ObjectId(id)}, {'$set': {'firesmoke_data': [], 'ai_solution':finddata['ai_solution']}})
#                                 if result.modified_count > 0:
                    ...
                    if len(firesmoke_data) != 0:
                        ...
                        if len(fetchfiresmoke_data) != 0:
                            if len(fetchfiresmoke_data) == 1:
                                result = mongo.db.ppera_cameras.update_one( {'_id': ObjectId(id)}, {'$set': {'firesmoke_data': firesmoke_data}})
                                if result.modified_count > 0:
                                ...
                                if len(firesmoke_data) == 1:
                                    for __, i in enumerate(fetchfiresmoke_data):
                                        if int(i['preset_key_id']) == int(firesmoke_data[0][ 'preset_key_id']):
                                            ...
                                            update_data.append(i)
                                    result = (mongo.db.ppera_cameras.update_one({'_id': ObjectId(id)}, {'$set': {'firesmoke_data': update_data}}))
                                    if result.modified_count > 0:
                                        ret = {'message': 'fire, smoke and dust data updated successfully.','success': True}
                                    else:
                ...
                if finddata is not None:
                    if presetkeyid is not None:
                        ...
                        if len(firesmoke_data) != 0:
                            ...
                            if len(firesmoke_data) == 1:
                                print("finddata['ai_solution']",finddata['ai_solution'])
                                result = mongo.db.ppera_cameras.update_one( {'_id': ObjectId(id)}, {'$set': {'firesmoke_data.presets': [] }})
                                if result.modified_count > 0:
                                    ret = {'message':'roi data delete successfully.','success': True}
                                else:
                                    ret['message'] = 'roi not deleted.'
...
def downloadSamplesheetCamerasetting():
    try:
        ...
        if filename:
            ...
            return send_from_directory(os.getcwd() + '/smaple_files/', 'CameraSettingsSamplesheet.xlsx')
        else:
...
def SHUTDOWNdownloadSamplesheetCamerasetting():
    try:
        list_of_files = glob.glob(os.getcwd() + '/smaple_files/' +'/*')
        # print('list of elements from the folder---- ',list_of_files)
        latest_file = max(list_of_files, key=os.path.getctime)
        ...
        if filename:
            main_path = os.path.abspath(path)
            return send_from_directory(os.getcwd() + '/smaple_files/', 'TSK_Shutdown_job_sample_job_sheet.xlsx')
        else:
            return {'success': False, 'message': 'File is not found.'}
    except (NameError, RuntimeError, FileNotFoundError, AssertionError,
            AttributeError, EOFError, FloatingPointError, TypeError,
