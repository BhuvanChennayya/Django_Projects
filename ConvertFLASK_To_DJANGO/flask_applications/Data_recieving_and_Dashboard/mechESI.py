...
mechesi = Blueprint('mechesi', __name__)
...
#done for object
@mechesi.route('/Mechdelete_job_sheet/<id>', methods=['GET'])
def delete_job_sheet(id=None):
    ret = {'success': False, 'message':'Something went wrong, please try again later'}
    try: 
        if id is not None:
            find_result= mongo.db.mechjob_sheet.find_one({'_id': ObjectId(id)})
            if find_result is not None:
                result = mongo.db.mechjob_sheet.delete_one({'_id': ObjectId(find_result['_id'])})
...
@mechesi.route('/Mechcreate_HyDra_config', methods=['GET'])
...
def remove_same_panel_from_db():
    try:
        ...
        if sheet_data is not None:
            data = list(mongo.db.mechesi.find({'job_sheet_name': sheet_data['job_sheet_name'], 'token': sheet_data['token']}, sort=[('_id', pymongo.DESCENDING)]))
            if data is not None:
                if len(data) != 0:
                    for ios, check_empty_panel_data in enumerate(data):
                        ...
                        if check_empty_panel_data['type'] =='HT' or check_empty_panel_data['type'] =='ht':
                            if isEmpty(check_empty_panel_data['data']) :
                                ...
                                result = mongo.db.mechesi.update_one({'_id':ObjectId(id)}, {'$set':{'data': final_data}})
...
def panel_detection_function_for_image(panel_image_file):
        ...
        for ___12, panel_val in enumerate(main_list):
            if len(panel_val['panel_data']) != 0:
                require_panel_data = {'ip_address': panel_val['ip_adrs' ], 'camera_name': panel_val['camera_name'],'rtsp_url': panel_val['rtsp_url'],
                                        'panel_data':panel_val['panel_data'], 'ai_solution': panel_val['ai_solution'], 'roi_ra': panel_val['roi_ra']}
...
def update_cam_id(ESICAMUPDATE):
    ...
    for RRR, total_data in enumerate(require_data['rtsp_cam_name']):
        ...
        result_data = mongo.db.mechesi.find_one({'data.rtsp_url': total_rtsp_url, 'data.camera_name': total_data_camera_name})
        if result_data is not None:
            ...
            result = mongo.db.mechesi.update_one({'_id': ObjectId(id),'data.rtsp_url': total_rtsp_url}, {'$set':{'data':result_data['data']}})
            if result.matched_count > 0:
                pass    
    return '200'
...
def NewCameraid_Update(ESICAMUPDATE):
    for RRR, total_data in enumerate(ESICAMUPDATE):
        ...
        result_data = list(mongo.db.mechesi.find({'data.ip_address': Cameraaddress}))
        if len(result_data) !=0:
            for Indexid , EachcameraUpdata in enumerate(result_data):
                id = EachcameraUpdata['_id']
                result = mongo.db.mechesi.update_one({'_id': ObjectId(id)}, {'$set':{'data.camera_id':total_data['camera_id']}})
                if result.matched_count > 0:
                    pass    
    return '200'
#gpu_configurations
...
def createNEWESIconfig():
    ...
    sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1}, sort=[('_id', pymongo.DESCENDING)])
    if sheet_data is not None:
        fetch_panel_data = list(mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token'],'type':"HT"}))
            ...
            for ol__io, i in enumerate(fetch_panel_data):
                ...
                if len(return_data) != 0:
                    ...
                    if len(panel_data) != 0:
                        ...
                        if len(panel_data) != 0:
                            if rtsp is not None:
                                require_panel_data = {'ip_adrs': ip_address, 'camera_name': cam_name,'rtsp_url': rtsp,'video_names':video_names,'panel_data':panel_data, 'ai_solution': ['PPE'],'roi_ra': []}
            ...
            with open(sample_config_file) as file:
                ...
                for young_TIGER, line in enumerate(file):
                    if line.strip() == '[application]':
                        ...
                        lines.append('rw-process-on-gpu = 1')
                        lines.append('gpu-id = {0}'.format(GPUSINDEX))
                    else:
...
@mechesi.route('/Mechcreate_esi_multiconfig', methods=['GET'])
...
@mechesi.route('/Mechstop_phaseoneapp', methods=['GET'])
...
@mechesi.route('/Mechstop_app_esi', methods=['GET'])
...
def verify_rtsp(type, video_name, camera_ip_address, url):
                            ...
                            if frame.shape[0] > 10 and frame.shape[1] > 10:
                                if frame.dtype == 'uint8':
                                    ...
                                    if type == 'ht' or type == 'HT':
                                        panel_detection_data = (panel_detection_function_for_image(full_path1))
...
def for_all_panel_id(type, video_name, ipaddress,camera_brand, camera_username,camera_password):
    data_append = []
    data = {}
    if camera_username == '':
    ...
    print('RTSP ------------------------------ ', rtsp_url)
    image_data = verify_rtsp(type, video_name, ipaddress, rtsp_url)
    if video_name is not None and video_name !='':
        if 'rtsp' in video_name:
            rtsp_url = video_name
        else:
...
def JOBrawdata(row):
    if row is not None:
        if row['ip_address'] is not None:
            return_data = for_all_panel_id(row['type'], row[ 'video_names'],  row['ip_address'],row['camera_brand'],row['camera_username'],row['camera_password'])
...
def GETJOBSHEETDETAILS(job_sheet_name,token_key):
    try:
        if job_sheet_name is not None:
            ...
            row['token'] =token_key
            row['reset_time'] = None
            result = mongo.db.mechjob_sheet.insert_one(row)
...
def read_the_excel_file_data_insert_to_db_new_concept(csv_creating_file,jobsheetfile, df, token):
    print(df)
            ...
            for index, eachrow in enumerate(ALL_DATA):
                ...
                if eachrow['ip_status']['status'] :                        
                    ...
                    if eachrow['ip_status']['status'] == True:
                        find_data = None#mongo.db.mechesi.find_one({ 'job_sheet_time':{'$regex': '^' + str(date.today())}, 'job_sheet_name': eachrow['job_sheet_name'],'token': token})#'job_no': eachrow['job_no'],
                        if find_data is None:
                            eachrow = JOBrawdata(eachrow)
                            if eachrow['data'] is not None:
                                if isEmpty(eachrow['data']) :
                                    ...
                                    result = mongo.db.mechesi.insert_one(eachrow)
                                    if result.acknowledged > 0:
                                        pass
                                    if mongo.db.mechjob_sheet.find_one({'job_sheet_name': eachrow['job_sheet_name'],'token': token}) is None:
                        ...
                        if 'feeder_number' in eachrow :
                            eachrow['panel']= eachrow['feeder_number']
                        result = mongo.db.mechesi.insert_one(eachrow)
                        find_data = mongo.db.mechesi.find_one({ 'job_sheet_time':{'$regex': '^' + str(date.today())}, 'job_no': eachrow['job_no'],'job_sheet_name': eachrow['job_sheet_name'],'token': token})
                        if find_data is None:
                            eachrow['data'] = {}    
                        if mongo.db.mechjob_sheet.find_one({'job_sheet_name': eachrow['job_sheet_name'],'token': token}) is None:
                            GETJOBSHEETDETAILS(jobsheetfile,token)   
    ...
    try:
        try:
            ...
            try:
                ...
                print("")
                #ERRORLOGdata(" ".join(["\n", "[ERROR] dashboard_apis -- upload666_file 2", str(error), " ----time ---- ", now_time_with_time()]))
        # df = pd.read_excel(file_path)
...
@mechesi.route('/MechESIupload_file', methods=['POST'])
def file_upload():
    ...
    if len(missing_key) != 0:
        ...
        app_run_response = SET_HYDRAAPP_monitoring_started(True)
        ...
        columns_to_check = ['Job No', 'Job Type', 'Department', 'Sub Area', 'Job Description',   'IP Address', 'Camera Brand', 'camera_username','camera_password']
                    ...
                    if 'video names' in missingcolumns:
                        ...
                        if get_excel_file.filename.endswith(tuple(Extension_excel)):
                            ...
                            if CHECKCAMERASANDLICENSEFOrJObsheet(get_excel_file)== True or CHECKCAMERASANDLICENSEFOrJObsheet(get_excel_file)== None :
                                ...
                                #     ERRORLOGdata(f"\n[ERROR] dashboard_apis -- upload----_file 1: {str(error1)} ----time ---- {now_time_with_time()}")
                                #     try:
                                ...
                                #         ERRORLOGdata(f"\n[ERROR] dashboard_apis -- upload_--file 2: {str(error2)} ----time ---- {now_time_with_time()}")
                                ...
                                try:
                                    ...
                                    ERRORLOGdata(" ".join(["\n", "[ERROR] dashboard_apis -- upload_---file 1", str(error), " ----time ---- ", now_time_with_time()]))
                                    try:
                                ...
                                if check_the_ip_status(csv_creating_file,df):
                                    check_status_variable =  read_the_excel_file_data_insert_to_db_new_concept(csv_creating_file , get_excel_file.filename, df, token_key)
                                    print('uploading-completed----------')
                                    print('--------------------------------------------check_status_variable=====================',check_status_variable)
                                    if check_status_variable is None: 
                                        ...
                                        if check_status_variable['error']:
                                            ...
                                            sheet_data = list(mongo.db.mechjob_sheet.find({'status': 1}, sort=[('_id', pymongo.DESCENDING)]))
                                            if len(sheet_data) != 0:
                                                think = 0
                                                app_run_response = SET_HYDRAAPP_monitoring_started(True)
                                                for im, simin in enumerate(sheet_data):
                                                    ...
                                                    result = mongo.db.mechjob_sheet.update_one(filters, newvalues)
                ...
                if get_excel_file.filename.endswith(tuple(Extension_excel)):
                    ...
                    if CHECKCAMERASANDLICENSEFOrJObsheet(get_excel_file)== True or CHECKCAMERASANDLICENSEFOrJObsheet(get_excel_file)== None :
                        try:
                            ...
                            ERRORLOGdata(" ".join(["\n", "[ERROR] dashboard_apis -- upload_fil---e 1", str(error), " ----time ---- ", now_time_with_time()]))
                            try:
                                read_file = pd.read_excel(filename_db, engine='openpyxl')
                                read_file = read_file.applymap(lambda x: x.strip() if isinstance(x, str) else x)
                                for column in columns_to_lowercase:
                                ...
                                ERRORLOGdata(" ".join(["\n", "[ERROR] dashboard_apis -- upload---_file 2", str(error), " ----time ---- ", now_time_with_time()]))
                        df = pd.DataFrame(pd.read_csv(csv_creating_file + '.csv'))  
                        df.drop(df.columns[df.columns.str.contains('unnamed',case = False)],axis = 1, inplace = True)          
                        if check_the_ip_status(csv_creating_file,df):
                            check_status_variable =  read_the_excel_file_data_insert_to_db_new_concept(csv_creating_file , get_excel_file.filename, df, token_key)
                            print("-----check_status_variable----1----------------",check_status_variable)
                            print('uploading-completed----------')
                            if check_status_variable is None: 
                                ret = {'message': 'successfully added', 'success': True}
                            else:
...
def getjobnumberslist(job_sheet_name,token):
    ...
    getdatajobnumberdata = list(mongo.db.mechesi.find({'job_sheet_name':job_sheet_name, 'token':token}))
    jobnumberlist = []
    ...
    try:
        try:
            ...
            try:
                ...
                print("")
                #ERRORLOGdata(" ".join(["\n", "[ERROR] dashboard_apis -- uploa----d_file 2", str(error), " ----time ---- ", now_time_with_time()]))
        # df = pd.read_excel(file_path)
...
@mechesi.route('/MechAddJobsViasheet', methods=['POST'])
def Addjobviasheet():
    # from Data_recieving_and_Dashboard.app import DetectPanel
    # detector = DetectPanel()
    detector= None
    ...
    if sheet_data is not None:
        ...
        if len(missing_key) != 0:
            ...
            get_excel_file.seek(0)
            get_excel_file.save(os.path.join(os.getcwd(),excelfilepath,filename_db))#os.path.join(os.getcwd(),excelfilepath,filename_db)
            columns_to_check = ['Job No', 'Job Type', 'Department', 'Sub Area', 'Job Description'
           , 'IP Address', 'Camera Brand','camera_username','camera_password']
            missingcolumns = check_excel_columns(get_excel_file, columns_to_check)
                    ...
                    if type(missingcolumns) ==list:
                            ...
                            if 'video names' in missingcolumns:                             
                                ...
                                if get_excel_file.filename.endswith(tuple(Extension_excel)):
                                    ...
                                    try:
                                        ...
                                        ERRORLOGdata(" ".join(["\n", "[ERROR] dashboard_apis -- upload_=tt---file 1", str(error), " ----time ---- ", now_time_with_time()]))
                                        try:
                                            read_file = pd.read_excel(filename_db, engine='openpyxl')
                                            read_file = read_file.dropna(how='all')
                                            read_file = read_file.applymap(lambda x: x.strip() if isinstance(x, str) else x)
                                            ...
                                            ERRORLOGdata(" ".join(["\n", "[ERROR] dashboard_apis -- uploatttd_file 2", str(error), " ----time ---- ", now_time_with_time()]))
                                    df = pd.DataFrame(pd.read_csv(csv_creating_file + '.csv'))  
                                    # df = df.applymap(lambda x: x.strip() if isinstance(x, str) else x)
                                    # df = df.applymap(lambda x: x.strip() if isinstance(x, str) else x)
                                    df.drop(df.columns[df.columns.str.contains('unnamed',case = False)],axis = 1, inplace = True)          
                                    if check_the_ip_status(csv_creating_file,df):
                                        check_status_variable =  read_the_excel_file_data_insert_to_db_new_concept(csv_creating_file , sheetname, df, token_key)
                                        print('uploading-completed----------')
                                        if check_status_variable is None: 
                                            ret = {'message': 'successfully added', 'success': True}
                        ...
                        if get_excel_file.filename.endswith(tuple(Extension_excel)):
                            try:
                                ...
                                ERRORLOGdata(" ".join(["\n", "[ERROR] dashboard_apis -- upload---_file 1", str(error), " ----time ---- ", now_time_with_time()]))
                                try:
                                    read_file = pd.read_excel(filename_db, engine='openpyxl')
                                    read_file = read_file.applymap(lambda x: x.strip() if isinstance(x, str) else x)
                                    for column in columns_to_lowercase:
                                    ...
                                    ERRORLOGdata(" ".join(["\n", "[ERROR] dashboard_apis -- uplo---ad_file 2", str(error), " ----time ---- ", now_time_with_time()]))
                            df = pd.DataFrame(pd.read_csv(csv_creating_file + '.csv'))  
                            # df = df.applymap(lambda x: x.strip() if isinstance(x, str) else x)
                            df.drop(df.columns[df.columns.str.contains('unnamed',case = False)],axis = 1, inplace = True)          
                            if check_the_ip_status(csv_creating_file,df):
...
@mechesi.route('/Mechreset_jobsheet', methods=['GET'])
def RESETJOB_SHEET():
...
# done for object
@mechesi.route('/Mechsheet_ipaddress', methods=['GET'])
@mechesi.route('/Mechsheet_ipaddress', methods=['POST'])
def sheet_ipaddress():
    ...
    if request.method == 'GET':
        if sheet_data is not None:
            ...
            data = list(mongo.db.mechesi.aggregate([{'$match': match_data}, {'$sort':{'job_sheet_time': -1}}, 
...
## done for object
@mechesi.route('/MechsheetDepartmentlist', methods=['GET'])
@mechesi.route('/MechsheetDepartmentlist', methods=['POST'])
...
## done for object 
@mechesi.route('/Mechlistofjobtypes', methods=['GET'])
def JOBTYPESORTING():
    ret = {'success': False, 'message':'something went wrong with get  details api'}
    if request.method == 'GET':
        sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1}, sort=[('_id', pymongo.DESCENDING)])
...
#done for object 
@mechesi.route('/Mechupdate_panel_roi_id', methods=['POST'])
                    ...
                    if id is not None:
                        if image_name is not None:
                            if panel_id is not None:
                                panel_find_data = (mongo.db.mechesi.find_one({'_id': ObjectId(id), 'data.image_name': image_name, 'data.panel_data.roi_data.panel_key_id': panel_key_id, 'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token']}))
                                if panel_find_data is not None:
                                    if isEmpty( panel_find_data[ 'data']) :
                                        ...
                                        result = (mongo.db.mechesi.update_one(filters, newvalues))
                                        if (result.modified_count > 0):
...
## done for object 
@mechesi.route('/Mechadd_panel_roi_id', methods=['POST'])
                            ...
                            if image_name is not None:
                                if panel_id is not None:
                                    if panel_key_id is not None:
                                        panel_find_data = mongo.db.mechesi.find_one({'_id': ObjectId(id), 'data.image_name': image_name, 'job_sheet_name': sheet_data[ 'job_sheet_name'], 'token': sheet_data['token']})
                                        if panel_find_data is not None:
...
@mechesi.route('/Mechexcel_download', methods=['GET'])
def excel_result():
    try:
...
@mechesi.route('/Mechviolation_excel_download', methods=['GET'])
                ...
                for ios, check_empty_panel_data in enumerate(data):
                    ...
                    if check_empty_panel_data['type']=='HT' or check_empty_panel_data['type']=='ht':
                        if isEmpty(check_empty_panel_data['data']):
                            ...
                            id = check_empty_panel_data['_id']
                            result = mongo.db.mechesi.update_one({'_id':ObjectId(id)}, {'$set':{'data.panel_data': final_panel_data_for_updating}})
                            if result.matched_count > 0:
                                pass
        ...
        if data is not None:
            if data['type'] =='HT' or data['type'] =='ht':
                ...
                if isEmpty(data['data']) :
                    ...
                    id = data['_id']
                    result = mongo.db.mechesi.update_one({'_id': ObjectId(id)},{'$set':{'data': data['data']}})
                    if result.matched_count > 0:                        
                        panel_delete_return_value = True
                        # print("data====delete data===",data['data']['panel_data'])
                        # if len(data['data']['panel_data'])==0:
                        #     newresult = mongo.db.mechesi.delete_one({'_id': ObjectId(id)})                            
    ...
    if len(rtsp_exist) != 0:
        if type(rtsp_exist) == dict:
            if isEmpty(rtsp_exist['data']) :
                # founddata= mongo.db.mechesi.find_one({"token":rtsp_exist['token'],"data.rtsp_url":rtsp_exist['data']['rtsp_url']})
            ...
            for __, inser in enumerate(rtsp_exist):
                if isEmpty(inser['data']) :
                    # founddata= mongo.db.mechesi.find_one({"token":inser['token'],"data.rtsp_url":inser['data']['rtsp_url']})
                    # if founddata is not None:
                    if inser['data']['rtsp_url'] is not None:
                        rtsp_status = False
                        diction['_id'] = inser['_id']
...
def NEWCOOLLNNECXT__RTSPADDED(rtsp_exist):
    ...
    if len(rtsp_exist) != 0:
        if type(rtsp_exist) == dict:
            if isEmpty(rtsp_exist['data']) :
                founddata= mongo.db.mechesi.find_one({"token":rtsp_exist['token'],"data.rtsp_url":rtsp_exist['data']['rtsp_url']},sort=[('_id', pymongo.DESCENDING)])
                if founddata is not None :
            ...
            for __, inser in enumerate(rtsp_exist):
                if isEmpty(inser['data']) :
                    founddata= mongo.db.mechesi.find_one({"token":inser['token'],"data.rtsp_url":inser['data']['rtsp_url']},sort=[('_id', pymongo.DESCENDING)])
                    if founddata is not None:
                        if founddata['data']['rtsp_url'] is not None:
                            rtsp_status = False
                            diction['_id'] = founddata['_id']
...
@mechesi.route('/Mechadd_panel_rtsp', methods=['POST'])
def add_new_panel():
            ...
            if output:
                ...
                if 'tagname' in output and 'board' in output:
                    ...
                    if rtsp_url is not None:
                        ...
                        if sheet_data is not None:
                            panel_status_data = list(mongo.db.mechesi.find({'job_sheet_name': sheet_data['job_sheet_name'], 'token': sheet_data['token']}, sort=[('_id', pymongo.DESCENDING)]))
                                            ...
                                            if regex_pwd.search(password) == None:
                                                ...
                                                if ping_response:
                                                    ...
                                                    if rtsp_response_image:
                                                        ...
                                                        result = mongo.db.mechesi.insert_one(main_data)
...
@mechesi.route('/Mechadd_panel_New_rtsp', methods=['POST'])
...
@mechesi.route('/Mechadd_panel_ip', methods=['POST'])
...
@mechesi.route('/Mechadd_panelNew_ip', methods=['POST'])
...
#done for the object
@mechesi.route('/Mechdelete_riro_data/<key_id>', methods=['GET'])
...
@mechesi.route('/MechdeleteRemark/<key_id>', methods=['GET'])
...
@mechesi.route('/Mechedit_riro_data', methods=['POST'])
def edit_riro_data():
...
@mechesi.route('/Mechdelete_panel', methods=['POST'])
...
@mechesi.route('/Mechriro_all_jobs_data_by_panel_id', methods=['POST'])
...
@mechesi.route('/Mechmultiisolation', methods=['GET'])
@mechesi.route('/Mechmultiisolation/<id>', methods = ['GET'])
def multii44solation_convayor_pnumertic_hydralic23323(id = None):
    ...
    # try:
        ...
        sheet_data=None
        if id is not None:
            sheet_data = mongo.db.mechjob_sheet.find_one({'_id': ObjectId(id)},sort=[('_id', pymongo.DESCENDING)])
        else:            
            sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])     
        if sheet_data is not None:
            data = list(mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token']}, sort=[('_id',   pymongo.DESCENDING)]))
...
@mechesi.route('/MechGetShutdownname', methods = ['GET'])
...
@mechesi.route('/MechUpdateShutdownname', methods = ['POST'])
def MechUpdateShutdownname():
    ret = {"message":"somthing wrong with job sheet ", 'success':False}
    jsonobject = request.json
    if jsonobject == None:
        jsonobject = {}
    ...
    if not missing_key:
        ...
        if output:
            ...
            if id is not None: 
                ...
                sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
            if sheet_data is not None:
                result = mongo.db.mechjob_sheet.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"shutdownname":shutdownname}})
                print(result.matched_count)
                if result.matched_count > 0:
...
@mechesi.route('/MechSORTINGIPWISE/<ip_address>', methods=['GET'])
    ...
    # try:
        ...
        if sheet_data is not None:
            if ip_address is not None:
                data = list(mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token'],"data.ip_address": ip_address}, sort=[('_id',   pymongo.DESCENDING)]))
                # print("data=====",len(data))
                if len(data) !=0 :
                    final_panel_data = []   
                    data = MUPANRRILATION_multi_isolation(data)
    ...
    if 1:
        ...
        if sheet_data is not None:
            if department is not None and ip_address is not None and panel_no is not None:
                data = mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token'],'department':department,"data.ip_address": ip_address,"data.panel_data":{"$elemMatch":{"panel_id": panel_no}}}, sort=[('_id',   pymongo.DESCENDING)])
            elif department is not None and ip_address is not None :
                data = mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token'],'department':department,"data.ip_address": ip_address}, sort=[('_id',   pymongo.DESCENDING)])
            elif department is not None  and panel_no is not None:
                data = mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token'],'department':department,"data.panel_data":{"$elemMatch":{"panel_id": panel_no}}}, sort=[('_id',   pymongo.DESCENDING)])
            elif ip_address is not None and panel_no is not None:
                data = mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token'],"data.ip_address": ip_address,"data.panel_data":{"$elemMatch":{"panel_id": panel_no}}}, sort=[('_id',   pymongo.DESCENDING)])
            elif department is not None :
                data = mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token'],'department':department}, sort=[('_id',   pymongo.DESCENDING)])
            elif  ip_address is not None :
                data = mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token'],"data.ip_address": ip_address}, sort=[('_id',   pymongo.DESCENDING)])
            elif  panel_no is not None :
                data = mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token'],"data.panel_data":{"$elemMatch":{"panel_id": panel_no}}}, sort=[('_id',   pymongo.DESCENDING)])
            else:
                ret['job_sheet_status']=True
...
###done object
@mechesi.route('/MechSORTINGJOBSHEETDATA', methods=['POST'])
...
def SORTJOBS(Query):
    if 1:
        ...
        if sheet_data is not None:
            ...
            data = list(mongo.db.mechesi.find(Query))
            # data = list(data)
...
@mechesi.route('/MechSORTINGBYJOBTYPE', methods=['POST'])
...
@mechesi.route('/MechGROUPBYPANEL', methods=['POST'])
def GROUPBYPANEL():
    ret = {'success': False, 'message':'something went wrong with SORTINGJOBSHEETDATA api'}
    data = request.json
    if data == None:
...
def TSKSHEET123(list1):
    ...
    # try:
        ...
        worksheet.write(row, col + 7, 'Switch Board Name', cell_format_1)
...
@mechesi.route('/Mechcreate_excel', methods=['GET'])
@mechesi.route('/Mechcreate_excel/<id>', methods=['GET'])
...
@mechesi.route('/Mechall_riro_data_history_new_concept', methods=['GET'])
...
@mechesi.route('/Mechriro_violation_data', methods=['POST'])
...
#done for object
@mechesi.route('/Mechget_panel_data1/<id>', methods=['GET'])
@mechesi.route('/Mechget_panel_data1/<id>/<imagename>', methods=['GET'])
    ...
    # try:
        ...
        if sheet_data is not None:
            ...
            if id is not None and imagename is not None:
                data = mongo.db.mechesi.find_one({'_id': ObjectId(id),'data.image_name': imagename, 'job_sheet_name': sheet_data['job_sheet_name'], 'token': sheet_data['token']}, sort=[('_id',pymongo.DESCENDING)])
                ...
                if data is not None:  
                    ...
                    if data['type']=='HT' or data['type']=='ht':
                        # NewDataOFall = list(mongo.db.mechesi.find({'ip_address':data['ip_address'] ,'job_sheet_name': sheet_data['job_sheet_name'], 'token': sheet_data['token']}, sort=[('_id',pymongo.DESCENDING)]))
                ...
                data = mongo.db.mechesi.find_one({'_id': ObjectId(id), 'job_sheet_name': sheet_data['job_sheet_name'], 'token': sheet_data['token']}, sort=[('_id',pymongo.DESCENDING)])
                if data is not None:      
                    if 'panel' in data:
                        newlapp.append(data['panel'])
                    elif 'feeder_number' in data:
...
## done for object
@mechesi.route('/Mechget_panel_data3/<id>/<imagename>', methods=['GET'])
...
def Panel_NEWIMAGE(data):
    ...
    if 1:
    # try:
        NewDataOFall = list(mongo.db.mechesi.find({'ip_address':data['ip_address'] ,'job_sheet_name': data['job_sheet_name'], 'token': data['token']}, sort=[('_id',pymongo.DESCENDING)]))
        # if list(NewDataOFall) !=0:
            # for i , jjin in enumerate(NewDataOFall):
            #     newlapp.append(jjin['panel'])
...
@mechesi.route('/MechrecaptureImageJob/<id>', methods=['GET'])
def recaptureImageJob(id=None):
    ret = {'success': False, 'message':'Something went wrong, please try again later'}
    if 1:
    # try:
        sheet_data = mongo.db.mechjob_sheet.find_one({},sort=[('_id', pymongo.DESCENDING)])
        if sheet_data is not None:
            if id is not None :
                data = mongo.db.mechesi.find_one({'_id': ObjectId(id)}, sort=[('_id', pymongo.DESCENDING)])
                if data is not None:
                    return_data = Panel_NEWIMAGE(data)
                    if return_data is not None:
                        print("999999999999999988888888888888888888899999999999999999999999999999999")
                        result = mongo.db.mechesi.update_one({ '_id': ObjectId(id)},  {'$set': {"data":return_data['data']}})
                        print(result.matched_count)
...
@mechesi.route('/Mechget_roi_image/<image_file>', methods=['GET'])
...
@mechesi.route('/Mechget_job_sheet_status', methods=['GET'])
@mechesi.route('/Mechget_job_sheet_status/<id>', methods=['GET'])
def get_job_she333et_status_of_current(id = None):
    ...
    # try:
        ...
        if sheet_data is not None:
            riro_return_data = []
            data1 = list(mongo.db.mechesi.find({'job_sheet_name': sheet_data['job_sheet_name'], 'token': sheet_data['token']}, sort=[('_id', pymongo.DESCENDING)]))
            if len(data1) != 0:
                ...
                for ___12, iisddf in enumerate(all_data):
                    if iisddf['type']=="HT" or iisddf['type']=='ht':
                        ...
                        if len(find_riro_data) != 0:
                            ...
                            iisddf['within_15_min']= None
                            iisddf['remarks'] = ''
                    elif iisddf['type']=='conveyor' or iisddf['type']=='CONVEYOR'  or iisddf['type']=="Pneumatic" or iisddf['type']=='pneumatic' or iisddf['type']=="Hydraulic" or iisddf['type']=='hydraulic'  :
                        #emmmi['type']=='conveyor' or emmmi['type']=='CONVEYOR' or emmmi['type']=='hydraulic' or emmmi['type']=='pneumatic' or emmmi['type']=='Hydraulic' or emmmi['type']=='Pneumatic':
                        hydata , panel_status = hydralockdataFetch(iisddf)
                        check_data = [{'panel_status': panel_status,'hydra_data': hydata}]
                        iisddf['riro_data'] = check_data
                        iisddf['riro_edit_status'] = False
                        iisddf['live_status'] = False
                        iisddf['sort_id'] = None
                        iisddf['isolation_status'] = None
                        iisddf['exception_status'] = False
                        # riro_final.append(iisddf)
                        # iisddf['sort_id'] = None
                        # iisddf['rack_method'] = None
                        # iisddf['rack_process'] = None
                        # iisddf['irrd_in_time'] = None
                ...
                if len(riro_return_data) != 0:
                    for singsing, xml_to in enumerate(riro_return_data):
                        ...
                        if xml_to['type']=="HT" or xml_to['type']=='ht':
                        ...
                        elif xml_to['type']=='conveyor' or xml_to['type']=='CONVEYOR'  or xml_to['type']=="Pneumatic" or xml_to['type']=='pneumatic' or xml_to['type']=="Hydraulic" or xml_to['type']=='hydraulic'  :
                            if len(xml_to['riro_data']) !=0:
                                if len(xml_to['riro_data']) ==1:
                                    if len(xml_to['riro_data'][0]['hydra_data']) !=0:
                                        # print("len(xml_to['riro_data']['hydra_data'])",xml_to['riro_data'][0]['hydra_data'])
                                        if len(xml_to['riro_data'][0]['hydra_data'])==1:
                                            checkhydradata = xml_to['riro_data'][0]['hydra_data'][0]
                                            if 'lock_on_details' in checkhydradata:
                                                if checkhydradata['lock_on_details'] is not None and  checkhydradata['lock_on_details'] is not None:
                                                    rack_method_done_count +=1
                                                elif checkhydradata['lock_on_details'] is not None:
                                                    pending_rack_ou_count +=1
                                                elif checkhydradata['lock_on_details'] is not None:
                                                    rack_method_done_count +=1
                                        else:
                                            allhydradata = xml_to['riro_data'][0]['hydra_data']
                                            for newindex , Newvalueofhydra in enumerate(allhydradata):
                                                if 'lock_on_details' in Newvalueofhydra:
                                                    if Newvalueofhydra['lock_on_details'] is not None and  Newvalueofhydra['lock_on_details'] is not None:
                                                        rack_method_done_count +=1
                                                        break
                                                    elif Newvalueofhydra['lock_on_details'] is not None:
                                                        pending_rack_ou_count +=1
                                                        break
                                                    elif checkhydradata['lock_on_details'] is not None:
                                                        rack_method_done_count +=1
                            ...
                            rack_method_not_done_count += 1
                    final_data = {'total_panel_count': total_panel_count,'processed_count': rack_method_done_count,'not_processed': rack_method_not_done_count,'working_progress':pending_rack_ou_count}
...
@mechesi.route('/Mechget_samplefile', methods=['GET'])
def get_obj_img():
    try:
...
@mechesi.route('/Mechriro_image/<image_file>', methods=['GET'])
...
@mechesi.route('/Mechtaglock_image/<image_file>', methods=['GET'])
...
@mechesi.route('/Mechfivemeter_image/<image_file>', methods=['GET'])
...
@mechesi.route('/Mechmagneticflasher_image/<image_file>', methods=['GET'])
...
## done for object
@mechesi.route('/Mechcheck_mechanical_job', methods=['POST'])
def mechanical_job_():
    ret = {'success': False, 'message':'Something went wrong, please try again later'}
    try:
        jsonobject = request.json
...
## done for object
@mechesi.route('/Mechmech_jobimage/<image_file>', methods=['GET'])
...
#done for object
@mechesi.route('/Mechdelete_mechanical_job/<id>', methods=['GET'])
def mechanical_job_delete(id=None):
    ...
    try: 
        ...
        if sheet_data is not None:
            if id is not None:
                find_result= mongo.db.mechesi.find_one({'_id': ObjectId(id),'job_sheet_name': sheet_data['job_sheet_name'], 'token': sheet_data['token']})
                if find_result is not None:
                    result = mongo.db.mechesi.delete_one({'_id': ObjectId(find_result['_id'])})
                    if result.deleted_count > 0:
...
@mechesi.route('/Mechhydra_history', methods=['POST'])
...
@mechesi.route('/Mechhydralockimage/<image_file>', methods=['GET'])
...
@mechesi.route('/Mechdelete_hydradata/<key_id>', methods=['GET'])
...
@mechesi.route('/Mechfetchdataflasher', methods=['GET'])
@mechesi.route("/fetchdataflasher/<ipaddress>/<panelnumber>",methods =['GET'])
@mechesi.route('/Mechfetchdataflasher/<panelnumber>', methods=['GET'])
...
#done for object
@mechesi.route('/Mechdelete_ht_job/<id>', methods=['GET'])
...
@mechesi.route('/Mechunplanned_data', methods=['GET'])
@mechesi.route('/Mechunplanned_data/<id>', methods=['GET'])
...
def GetthejobshettTagnames():
    ...
    if sheet_data is not None:
        ...
        data = list(mongo.db.mechesi.find({'job_sheet_name':sheet_data['job_sheet_name'], 'token': sheet_data['token'],'type':{"$in":['ht','HT']}}, sort=[('_id',   pymongo.DESCENDING)]))
        if len(data) !=0:
    ...
    return Tagnamelist
@mechesi.route('/MechGetL1dataofHT', methods=['GET'])
def FETCHL1dataofHT():
...
@mechesi.route('/MechGetUnplannedLivecount', methods=['GET'])
...
@mechesi.route('/Mechdelete_unplannedRiro/<key_id>', methods=['GET'])
...
@mechesi.route('/Mechedit_UnplannedRIROData', methods=['POST'])
...
@mechesi.route('/MechDeleteFeedernumber', methods = ['POST'])
    ...
    if not missing_key:
        ...
        if output:
            ...
            if id is not None:
                ...
                if find_edit_data is not None: 
                #sheet_data = mongo.db.mechesi.find_one({'_id': ObjectId(id)},sort=[('_id', pymongo.DESCENDING)])
            # else:
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                # if sheet_data is not None:
...
@mechesi.route('/MechUnplannedriroimage/<image_file>', methods=['GET'])
...
@mechesi.route('/MechRiroUnplannedviolation_verification/<id>/<flag>', methods=['GET'])
...
@mechesi.route('/MechUnplannedJobscreate_excel', methods=['GET'])
@mechesi.route('/MechUnplannedJobscreate_excel/<id>', methods=['GET'])
...
@mechesi.route('/MechUnplannedexcel_download', methods=['GET'])
...
@mechesi.route('/MechUpdateTagName', methods = ['POST'])
def UpdateTagName():
    ret = {"message":"somthing wrong with job sheet ", 'success':False}
    jsonobject = request.json
    if jsonobject == None:
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"tagname":tagname}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechDeleteTagName', methods = ['POST'])
def DeleteTagName():
    ret = {"message":"somthing wrong with job sheet ", 'success':False}
    jsonobject = request.json
    if jsonobject == None:
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"tagname":None}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechUpdateAreaName', methods = ['POST'])
def UpdateAreaname():
    ret = {"message":"somthing wrong with job sheet ", 'success':False}
    jsonobject = request.json
    if jsonobject == None:
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"sub_area":sub_area}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechDeleteAreaName', methods = ['POST'])
def DeleteAreaName():
    ret = {"message":"somthing wrong with job sheet ", 'success':False}
    jsonobject = request.json
    if jsonobject == None:
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"sub_area":None}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechUpdateDepartmentname', methods = ['POST'])
def UpdateDepartmentname():
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"department":department}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechDeleteDepartmentname', methods = ['POST'])
def DeleteDepartmentname():
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"department":None}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechUpdateJobDescription', methods = ['POST'])
def UpdateJobDescription():
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"job_description":job_description}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechDeleteJobDescription', methods = ['POST'])
def DeleteJobDescription():
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"job_description":None}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
#SwitchBoardName
@mechesi.route('/MechUpdateSwitchBoardName', methods = ['POST'])
def UpdateSwitchBoardName():
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"board":boardname}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechDeleteSwitchBoardName', methods = ['POST'])
def DeleteSwitchBoardName():
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"board":None}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechGetJOBNUMBER', methods=['GET'])
@mechesi.route('/MechGetJOBNUMBER', methods=['POST'])
def GetJOBNUMBER():
    try:
        ret = {'success': False, 'message':'something went wrong with get  details api'}
        sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1}, sort=[('_id', pymongo.DESCENDING)])
...
@mechesi.route('/MechAddRemarksForHoleJob', methods = ['POST'])
def AddRemarksForHoleJob():
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"remark":remark}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechRemoveRemarksForHoleJob', methods = ['POST'])
def RemoveRemarksForHoleJob():
    ...
    if not missing_key:
        ...
        if output:
            ...
            #     sheet_data = mongo.db.mechjob_sheet.find_one({'status': 1},sort=[('_id', pymongo.DESCENDING)])
                if sheet_data is not None:
                    result = mongo.db.mechesi.update_one({ '_id': ObjectId(sheet_data['_id'])},  {'$set': {"remark":None}})
                    print(result.matched_count)
                    if result.matched_count > 0:
...
@mechesi.route('/MechRemoveRemarksForUnplannedJob', methods = ['POST'])
...
@mechesi.route('/Mechadd_hydraulic', methods=['POST'])
def Mechadd_1hydraulic():
                ...
                if isEmpty(list_data) :
                    ...
                    if sheet_data is not None:
                        ...
                        if id is not None:
                            if image_name is not None:
                                if panel_key_id is not None:
                                    panel_find_data = mongo.db.mechesi.find_one({'_id': ObjectId(id), 'data.image_name': image_name, 'job_sheet_name': sheet_data[ 'job_sheet_name'], 'token': sheet_data['token'],'type':{"$ne": "HT"}})
                                    if panel_find_data is not None:
                                        panel_find_data_roi = panel_find_data[ 'data']
                                        if isEmpty(panel_find_data_roi) :
...
@mechesi.route('/Mechdelete_mechanical_job/<id>', methods=['GET'])
def Mechdelete_mechanical_job(id = None):
    ret ={'message':'something went wrong with delete mechanical','success':False}
    if 1:
        if id is not None:
            x = mongo.db.mechesi.find_one({'_id': ObjectId(id)})
            if x is None:
                ret['message'] = 'no data found for the deletion.'
            else:
                result = mongo.db.mechesi.delete_one({'_id': ObjectId(id)})
                if result.deleted_count > 0:
                    ret = {'message': 'riro_data deleted successfully.','success': True}
                else:
                    ret['message'] = 'riro_data is not deleted ,due to something went wrong with database.'
...
@mechesi.route('/Mechdelete_pneumatic', methods=['POST'])
def Mechdelete_pneumatic():
    ret = {'success': False, 'message':'Something went wrong, please try again later'}
    if 1:
        jsonobject = request.json
        if jsonobject == None:
        ...
        if not missing_key:
            ...
            if output:
                ...
                if id is not None:
                    data = mongo.db.mechesi.find_one({'_id':ObjectId(id)})
                    if data is not None:
                        return_value_for_delete_panel = Mechremove_delete_pneumatic_from_db(data, imagename, panel_key_id)
                        if return_value_for_delete_panel:
...
def Mechremove_delete_pneumatic_from_db(data, image_name, panel_key_id):
    panel_delete_return_value = False
        ...
        if data is not None:
            if isEmpty(data['data']):
                ...
                if length_measure:
                    ...
                    result = result = mongo.db.mechesi.update_one({'_id': ObjectId(id)},{'$set':{'data': data['data']}})
                    if result.matched_count > 0:
...
def Mechremove_delete_hydraulic_from_db(data, image_name, panel_key_id):
    panel_delete_return_value = False
    panel_delete_return_value = {'message':"initial message",'success':False}
    length_measure = False
    if 1:
...
@mechesi.route('/Mechdelete_hydraulic', methods=['POST'])
def Mechdelete_hydraulic():
    ret = {'success': False, 'message':'Something went wrong, please try again later'}
    if 1:
    # try:
        jsonobject = request.json
        ...
        if not missing_key:
            ...
            if output:
                ...
                if id is not None:
                    data = mongo.db.mechesi.find_one({'_id':ObjectId(id)})
                    if data is not None:
                        return_value_for_delete_panel = Mechremove_delete_hydraulic_from_db(data, imagename, panel_key_id)
                        if return_value_for_delete_panel:
                            if return_value_for_delete_panel['success']:
...
@mechesi.route('/Mechadd_pneumatic', methods=['POST'])
def Mechadd_pneumatic():
